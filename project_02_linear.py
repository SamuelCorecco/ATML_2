...
def save_checkpoint(model, optimizer, epoch, train_losses, valid_losses, num_epochs, batch_size):
    ...
    filename=f"checkpoint-linear-e-{num_epochs}-b-{batch_size}.pkl"
...
class LinearSeq2SeqModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim):
        super(LinearSeq2SeqModel, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim, padding_idx=0)

        # Replace LSTM with Linear layers
        self.encoder = nn.Linear(embedding_dim, hidden_dim)
        self.decoder = nn.Linear(embedding_dim + hidden_dim, hidden_dim)
    ...
    def forward(self, encoder_input, decoder_input):
        ...
        encoder_output = torch.relu(self.encoder(encoder_embedded))  # Apply non-linearity

        # Summarize encoder output (e.g., mean pooling)
        encoder_summary = encoder_output.mean(dim=1)  # Shape: [batch_size, hidden_dim]

        # Embed the decoder input
        decoder_embedded = self.embedding(decoder_input)
        # Concatenate decoder embeddings with the encoder summary
        decoder_combined = torch.cat(
            [decoder_embedded, encoder_summary.unsqueeze(1).expand(-1, decoder_embedded.size(1), -1)],
            dim=-1
        )

        decoder_output = torch.relu(self.decoder(decoder_combined))
...
def generate_response(model, tokenizer, input_text, maxlen_questions, device, max_len=50):
    model.eval()
    input_seq = tokenize_and_pad([input_text], tokenizer, maxlen_questions).to(device).long()

    with torch.no_grad():
        encoder_embedded = model.embedding(input_seq)
        encoder_output = torch.relu(model.encoder(encoder_embedded))  # Linear encoder output
        ...
        for _ in range(max_len):
            ...
            # Combine decoder input with encoder output
            decoder_combined = torch.cat([decoder_embedded, encoder_output.unsqueeze(1)], dim=-1)
            decoder_output = torch.relu(model.decoder(decoder_combined))
...
def main(num_epochs, batch_size, train):
    # Load datasets
    splits = [
    ...
    model = LinearSeq2SeqModel(VOCAB_SIZE, embedding_dim, hidden_dim)
